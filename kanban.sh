...
invoke_reviewer() { # invoke_reviewer <card> <workdir> <prompt> -> sets ATT_SCORE/ATT_FEEDBACK/ATT_REVIEW_INFRA_ERROR
  ...
  local title rcmd review_out parsed
...
run_attempt() { # run_attempt <card> <workdir> <worker-infra-max> -> sets ATT_SCORE / ATT_FEEDBACK / ATT_BLOCKED_REASON
  ...
  local backend model wcmd out title infra_cat retries
  ATT_BLOCKED_REASON=""
...
run_resolve_attempt() { # run_resolve_attempt <card> <resolve-workdir> <conflict-files> <base-branch> <card-branch>
