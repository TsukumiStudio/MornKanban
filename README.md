...
If the reclaimed card blocks again with the **identical** reason `DEFAULT_ORDERING_STALL_REPEATS` times in a row (default 3; `KANBAN_ORDERING_STALL_REPEATS`), it can never resolve on its own (e.g. a card whose body is missing, so the worker always asks the same unanswerable question) -- the dispatcher instead records `blocked_kind: ordering_stalled`, stops reclaiming it on restart, and `kanban list` marks it as needing user judgment. Any change in the reason resets the count to 1, so a legitimate ordering wait that clears (or whose stated reason moves on) before hitting the limit is dispatched normally and never stalls. `kanban resume <id>` clears the block and the repeat count, returning it to `todo`; `kanban remove <id>` deletes it outright. Both work for `ordering` and `ordering_stalled` alike.
