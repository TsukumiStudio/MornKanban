...
- `kanban update` refuses tracked/staged changes, detached HEAD, or a non-`main` checkout, then runs `git pull --ff-only origin main` and reinstalls the CLI symlink and skills. Untracked files are preserved; Git still aborts if an incoming path would overwrite one.
